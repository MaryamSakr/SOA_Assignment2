...
async function saveData() {
  ...
  if (--studentNumber > 0) {
    ...
    await sendData();
    alert("All Students added succefully.");
...
function getNumberOfStudent(students, containerName) {
  const searchDiv = document.getElementById(`${containerName}`);
...
async function getStdFirstname() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      getNumberOfStudent(students, "studentNum");
    } else {
      console.error("Failed to fetch data:", response.statusText);
    }
  } catch (error) {
...
function displayStudents(students, ulname) {
  ...
  getNumberOfStudent(students, "studentNumDisplay");
}
